...
import { Modal, StyleSheet, Text, TouchableOpacity, View, ScrollView, StatusBar } from 'react-native';
import { useSelector, useDispatch } from 'react-redux';
import Icon from 'react-native-vector-icons/MaterialIcons';
import { typography } from '../theme/typography';
import { STATUS_BAR_HEIGHT } from '../theme/spacing';
import { selectCartSubtotal, selectCartGstTotal, clearServerCart } from '../Redux/slices/cartSlice';
...
// Cart drawer opened from the top bar on OnboardingProfile / OnboardingPayment
// — the registration steps a guest lands on after adding a service to their
// cart and then registering/signing in. Same cart as the guest Cart.js screen
// (same Redux state, same remove/empty/browse behavior) — only the chrome is
// a modal instead of a pushed screen, and the membership prompt reads
// "Continue to Membership Payment" instead of Sign In/Register, since the
// customer is already authenticated by this point (registerUser.fulfilled
// flips isAuthenticated immediately after signup). useCart() binds the
// authenticated cart endpoints: merges/fetches via GET /customer/cart on
// mount, remove() below hits DELETE /customer/cart/items/{serviceId}.
function OnboardingCartModal({ visible, onClose, navigation }) {
  ...
  // Remove-item / clear-cart confirm — { title, message, confirmLabel, onConfirm } | null.
  // Same interaction as Cart.js.
  ...
  const confirmRemoveItem = (item) => {
    setConfirmState({
      ...
      confirmLabel: 'Remove',
      onConfirm: () => remove(item.serviceId),
    });
  };

  // Always authenticated by this point (see docblock above) — clear via
  // DELETE for every server-cart row so nothing reappears on the next fetch.
  const confirmClearCart = () => {
    setConfirmState({
      title: 'Clear Cart',
      message: 'Remove all services from your cart?',
      confirmLabel: 'Clear Cart',
      onConfirm: () => dispatch(clearServerCart(items)),
    });
  };

  // Same target Cart.js uses ('GuestServices') — reached via the GuestHome
  // stack since this modal lives on the root stack (Onboarding screens),
  // not inside GuestStack itself.
  const goToServices = () => {
    onClose();
    navigation.navigate('GuestHome', { screen: 'GuestServices' });
  };
      ...
      <View style={styles.container}>
        ...
        {empty ? (
          <View style={styles.emptyWrap}>
            ...
            <TouchableOpacity style={styles.browseBtn} onPress={goToServices}>
          ...
          <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
            ...
            {/* Membership prompt — already registered by this screen, so this
                just returns to the onboarding step that carries you to
                membership payment, rather than Sign In / Register. */}
            ...
            <View style={styles.authCard}>
              ...
              <TouchableOpacity style={styles.continueBtn} onPress={onClose}>
                <Text style={styles.continueBtnText}>Continue to Membership Payment</Text>
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.summaryCard}>
              ...
              <TouchableOpacity style={styles.keepBrowsingBtn} onPress={goToServices}>
...
const styles = StyleSheet.create({
  ...
  trustRow: { flexDirection: 'row', justifyContent: 'space-between', gap: 8, marginTop: 8, marginBottom: 20 },
  trustItem: {
  ...
  continueBtn: { backgroundColor: '#F97316', borderRadius: 12, paddingVertical: 14, alignItems: 'center' },
  continueBtnText: { fontSize: 15, fontFamily: typography.h4.fontFamily, color: '#FFFFFF' },
...
export default OnboardingCartModal;
